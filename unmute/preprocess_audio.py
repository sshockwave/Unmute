...
MAX_FRAMES = int(1000*SAMPLE_LEN) # changed
...
def main():
	...
	if isfile(viddata_path) and isfile(auddata_path):
		...
		vidfiles = [f for f in listdir(datapath) if isfile(join(datapath, f)) and f.endswith(".mp4")]
		faceCascade = cv2.CascadeClassifier(CASC_PATH)
		vidctr = 0
		audctr = 0
		progress = 0
		...
		for vf in vidfiles:
			...
			audctr = process_audio(join(datapath,vf.replace('.mp4','.wav')), auddata, audctr)
